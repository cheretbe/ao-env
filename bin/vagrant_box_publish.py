...
def check_vagrant_cloud_login(batch_mode):
    ...
    if subprocess.call(
        ...
        if batch_mode:
            sys.exit("Will not use 'vagrant cloud auth login' in batch mode")
...
def publish_box( # pylint: disable=too-many-arguments
    ...
    vagrant_parameters = [
        ...
        box_version, "virtualbox", box_file,
...
def main():
    ...
    box_name, box_version = get_box_name_and_version(
        ...
        explicit_name=options.box_name,
    ...
    box_description = get_box_description(
        batch_mode=options.batch, is_new_box=(not bool(current_version))
    )
    ...
    print(
        ...
        f"version {new_version}" + current_version_info +
        f"\nVersion description: {version_description}"
    )
